...
<article>
  ...
  <tr>
	...
	<?php
		$sql="SELECT * FROM orders WHERE buyer_id='$userid'";
		$result = $conn->query($sql);
		foreach($result as $value){
			
			$productid=$value['product_id'];
			if($value['status']!='交易完成'){
			?>
			<tr>
			...
			?>
			<td width="10%"><a href="product.php?productid=<?php echo $value['product_id']; ?>" style="text-decoration:none;color:#0000FF"><?php echo $row['product_name']; ?></td>
			...
			if($value['payment']=='n'){
				?>
				<td width="5%"><a href="pay.php?orderid=<?php echo $value['orderid'];?>" style="text-decoration:none">付款</a></td>
				<?php
			}
			else{
				?>
			...
			?>
			<td width="5%"><a href="cancel.php?orderid=<?php echo $value['orderid'];?>" style="text-decoration:none;color:#0000FF">取消訂單</a></td>
			<td width="5%"><a href="transaction.php?orderid=<?php echo $value['orderid'];?>" style="text-decoration:none;color:#0000FF">完成交易</a></td>
			</tr>
			<?php
		}
	?>
