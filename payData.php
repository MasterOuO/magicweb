...
<article>
	...
	$sql="SELECT * FROM orders WHERE orderid='$orderid'";
		...
		if($money<0){
			echo "餘額不足 <br>";
			?><a href="store.php">前往儲值</a><br>
			<?php
		}
		else{
			$sqll="UPDATE user SET wallet='$money' WHERE account='$account'";
			$resule = $conn->query($sqll);
			$sellerid = $row1['seller_id'];
			$sqli="SELECT * FROM user WHERE id='$sellerid'";
			$resultt = $conn->query($sqli);
			$row2 = $resultt->fetch_assoc();
			
			$getmoney=$row2['wallet']+$row1['price'];
			$sqlll="UPDATE orders SET payment='y' WHERE orderid='$orderid'";
			$sqllll="UPDATE user SET wallet='$getmoney' WHERE id='$sellerid'";
			if($conn->query($sqlll)===TRUE&&$conn->query($sqllll)===TRUE){
				echo "成功付款<br>目前餘額: ",$money,"<br><br>";
			}
		}
	?>
</article>
