...
<nav>
	...
	<ul>
	<img src="head.jpg" alt="Kurumi" width="100" height="100">
...
<article>
	...
	if(!empty($product_name)){
		$sql="UPDATE product SET product_name='$product_name' WHERE product_id='$productid'";
		if($conn->query($sql)===TRUE){
			echo "產品名稱已更改為: ",$product_name,"<br><br>";
		}
	}
	if(!empty($stock)){
		$sql="UPDATE product SET stock='$stock' WHERE product_id='$productid'";
		if($conn->query($sql)===TRUE){
			echo "產品庫存已更改為: ",$stock,"<br><br>";
		}
	}
	if(!empty($one_price)){
		$sql="UPDATE product SET one_price='$one_price' WHERE product_id='$productid'";
		if($conn->query($sql)===TRUE){
			echo "產品價格已更改為: ",$one_price,"<br><br>";
		}
	}
	if(!empty($type)){
		$sql="UPDATE product SET type='$type' WHERE product_id='$productid'";
		if($conn->query($sql)===TRUE){
			echo "產品總類已更改為: ",$type,"<br><br>";
		}
	}
	if(!empty($introduct)){
		$sql="UPDATE product SET introduct='$introduct' WHERE product_id='$productid'";
		if($conn->query($sql)===TRUE){
			echo "產品介紹已更改為: ",$introduct,"<br><br>";
		}
	}
	?>
